...
const AddTestPanel = () => {
  ...
  const handleUpdatedClick = async () => {
    try {
      ...
      if (response.status === 201) {
        ...
        toast.success("test panel Added Successfully");
      } else {
  ...
  return (
    <div className="container bg-white p-6">
        ...
        <div className="mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="col-span-1">
              <input type="text" placeholder="Name" name="name" value={testpanel.name} onChange={handleInputChange} className="border rounded-lg px-3 py-2 w-full" />
            </div>
            <div className="col-span-1">
              <input type="number" placeholder="Price" name="price" value={testpanel.price} onChange={handleInputChange} className="border rounded-lg px-3 py-2 w-full" />
            </div>
      ...
      <select className="form-select" aria-label="Default select example" name="tests" onChange={(event) => { handleInputChange(event); addToList(event); }}>
  <option value="none">None</option>
  {tests.map((item, index) => (
    <option value={item} key={index}>
      {item}
        ...
        {/* <input type="text" placeholder="Topics" name="tests" value={testpackage.tests} onChange={handleInputChange} className="border rounded-lg px-3 py-2 w-full" /> */}
      </div>
